...
import gettext
gettext.install('pydiction', unicode=1)
...
def get_submodules(module_name, submodules):
    ...
    try:
        ...
        return submodules
    if imported_module.__file__.endswith("__init__.pyc"):
        files = os.listdir(imported_module.__file__.split('/__init__.pyc')[0])
        for f in files:
            f = module_name + '.' + f
            submodules = get_submodules(f, submodules)
